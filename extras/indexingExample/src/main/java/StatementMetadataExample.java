...
public class StatementMetadataExample {
     ...
     * metadata. In the example below, the query returns all the places that Joe
     * works at along with the people that created the triples containing those locations
     * and the dates that those triples were created on. This example is the same as example1,
     * but demonstrates that there is no need for the subject to be a blankNode when
     * using the StatementMetadata framework.  The subject can be a variable as well.
     * 
     * @throws Exception
     */
    private void example5() throws Exception {

        String query1 = "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> \n"
                + "PREFIX owl: <http://www.w3.org/2002/07/owl#> \n"
                + "SELECT ?x ?y ?z \n" 
                + "WHERE {\n"
                + "?a rdf:type owl:Annotation. \n" 
                + "?a owl:annotatedSource <http://Joe>. \n"
                + "?a owl:annotatedProperty <http://worksAt>. \n" 
                + "?a owl:annotatedTarget ?x. \n"
                + "?a <http://createdBy> ?y. \n" 
                + "?a <http://createdOn> ?z }\n";
