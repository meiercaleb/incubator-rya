...
public class CreateDeleteIT extends ITBase {
	...
	public void historicResults() throws Exception {
		...
		final String sparql = "SELECT ?x " + "WHERE { "
				+ "?x <http://talksTo> <http://Eve>. "
				+ "?x <http://worksAt> <http://Chipotle>." + "}";

		// Triples that are loaded into Rya before the PCJ is created.
		final Set<Statement> historicTriples = Sets
				.newHashSet(
						makeStatement("http://Alice", "http://talksTo",
								"http://Eve"),
						makeStatement("http://Bob", "http://talksTo",
								"http://Eve"),
						makeStatement("http://Charlie", "http://talksTo",
								"http://Eve"),

						makeStatement("http://Eve", "http://helps",
								"http://Kevin"),
		...
		expected.add(makeBindingSet(new BindingImpl("x", new URIImpl(
				"http://Bob"))));
		expected.add(makeBindingSet(new BindingImpl("x", new URIImpl(
				"http://Charlie"))));
...
//		new DeletePcj(1).deletePcj(client, pcjId);
	...
	public void streamedResults() throws Exception {
		...
		// created.
		final Set<RyaStatement> streamedTriples = Sets
				.newHashSet(
						makeRyaStatement("http://Alice", "http://talksTo",
								"http://Eve"),
						makeRyaStatement("http://Bob", "http://talksTo",
								"http://Eve"),
						makeRyaStatement("http://Charlie", "http://talksTo",
								"http://Eve"),

						makeRyaStatement("http://Eve", "http://helps",
								"http://Kevin"),
