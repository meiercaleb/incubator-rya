...
public class CommandNotificationEncoder implements Encoder<CommandNotification>, Decoder<CommandNotification> {

    private static Gson gson = new GsonBuilder()
            .registerTypeHierarchyAdapter(Notification.class, new CommandNotificationTypeAdapter()).create();
    private static final Logger LOG = LoggerFactory.getLogger(CommandNotificationEncoder.class);
    private VerifiableProperties props;

    public CommandNotificationEncoder() {
    };

    // this constructor is needed for use with Kafka producers
    public CommandNotificationEncoder(VerifiableProperties props) {
        this.props = props;
    }
